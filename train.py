...
for epoch in range(epochs):
    ...
    for i in range(len(ids)):
        ...
        # print(model.mu_fc)
        loss_graph_rec, loss_kl_disentangle, loss_kl = model(A_fc = fcs[i, :, :].float().to(device), A_sc = scs[i, :, :].float().to(device), age = ages_tensor[i].float().to(device), gender = gender[i].view(-1).float().to(device))
 
        loss = loss_graph_rec + loss_kl+ loss_kl_disentangle
        loss.backward()
        optimizer.step()
    ...
    print(str(epoch)+' loss:'+str(total_loss/len(ids))+' disentangle kl:'+str(total_dis_kl/len(ids))+' kl:'+str(total_kl/len(ids))+' rec:'+str(total_rec/len(ids))+' l1 norm:'+str(total_l1/len(ids)))
