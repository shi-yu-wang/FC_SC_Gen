...
class MolGen(nn.Module):
    ...
    def forward(self, A_fc, A_sc, age, gender):
        ...
        z_prop = torch.cat((torch.sum(z_fc, dim=0).view(1, -1), torch.sum(z_joint, dim=0).view(1, -1),
                            torch.sum(z_sc, dim=0).view(1, -1)), dim=1)
        ...
        log_qfsj, log_prod_qfqsqj = self.get_tc(z_fc, z_sc, z_joint, z_mu_fc, z_sigma_fc, z_mu_sc, z_sigma_sc,
                                                z_mu_joint, z_sigma_joint)
        loss_kl_disentangle = (log_qfsj - log_prod_qfqsqj).mean()

        latent_dist = (
        torch.cat([z_mu_fc, z_mu_sc, z_mu_joint], dim=-1), torch.cat([z_sigma_fc, z_sigma_sc, z_sigma_joint], dim=-1))
    ...
    def get_tc(self, z_fc, z_sc, z_joint, mu_fc, sigma_fc, mu_sc, sigma_sc, mu_joint, sigma_joint):
        ...
        mat_log_qzfsj = self.matrix_log_density_gaussian(latent_sample, torch.cat((mu_fc, mu_sc, mu_joint), dim=-1),
                                                         torch.cat((sigma_fc, sigma_sc, sigma_joint), dim=-1))
